        ...
        <div class="bg-gradient-to-br from-[#e92027] via-[#b91c1c] to-[#7f090b] text-white pb-32 pt-16 px-8 -mt-6 -mx-6 mb-8 rounded-b-[3rem] shadow-2xl relative overflow-hidden">
             ...
             <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center relative z-10 gap-6">
                <div class="text-center md:text-left">
                     ...
                     <p class="text-red-50 text-base font-light opacity-95 max-w-lg leading-relaxed drop-shadow-sm">Kelola data peminjaman arsip.</p>
                </div>
                <div>
                            ...
                            @forelse($peminjaman as $detail)
                                <tr class="hover:bg-red-50/20 transition duration-150 group">
                                    ...
                                    <td class="px-6 py-4 text-center border-r border-gray-100">
                                        @php
                                            // PRIORITIZE THE SNAPSHOT (hak_akses) which we now save
                                            $akses = $detail->hak_akses; 
                                            // Fallback to relation only if snapshot is empty (for old data)
                                            if (empty($akses) && $detail->arsip && $detail->arsip->klasifikasi) {
                                                $akses = $detail->arsip->klasifikasi->hak_akses;
                                            }

                                            $aksesClass = 'bg-orange-50 text-orange-700 border-orange-200'; // Default (Biasa)
                                            if ($akses === 'Rahasia') {
                                                $aksesClass = 'bg-red-50 text-[#c41820] border-red-200';
                                            } elseif ($akses === 'Terbatas') {
                                                $aksesClass = 'bg-yellow-50 text-yellow-700 border-yellow-200';
                                            } elseif ($akses === 'Sangat Rahasia') {
                                                $aksesClass = 'bg-[#e92027] text-white border-[#e92027]';
                                            }
                                        @endphp
                                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border {{ $aksesClass }}">
                                            {{ $akses ?? '-' }}
                                        </span>
                                    </td>

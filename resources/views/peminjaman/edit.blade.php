<x-layout>
    {{-- Header Page --}}
    {{-- Header Page --}}
    <div class="bg-[#9d1b1b] px-6 md:px-8 pt-6 pb-20 rounded-b-[2.5rem] shadow-xl mb-8 flex items-center justify-between relative overflow-hidden">
    ...
    <div class="max-w-5xl mx-auto px-6 -mt-20 relative z-20 mb-10" x-data="peminjamanEdit()">
        ...
        <form action="/peminjaman/{{ $editData->id }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm($el)" 
            ...
            <div class="p-6 md:p-8 space-y-8">
                ...
                <div>
                    <h2 class="text-lg font-bold text-[#9d1b1b] border-b border-gray-100 pb-3 mb-6 flex items-center gap-3">
                        <i class="fas fa-user-edit text-[#9d1b1b]"></i> Data Peminjam
                    </h2>

                    <div class="space-y-5">
                        {{-- Tanggal --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Tanggal Peminjaman <span class="text-red-600">*</span></label>
                            <input type="date" name="tanggal" value="{{ $editData->tanggal_pinjam }}" required 
                                class="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 outline-none transition bg-gray-50 focus:bg-white focus:border-[#9d1b1b] focus:ring-4 focus:ring-[#9d1b1b]/10 scale-100">
                        </div>

                        {{-- Nama --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Nama Peminjam <span class="text-red-600">*</span></label>
                        ...
                        {{-- NIP --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">NIP <span class="text-red-600">*</span></label>
                        ...
                        {{-- Jabatan --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Jabatan <span class="text-red-600">*</span></label>
                            <div class="relative">
                                <select name="jabatan_peminjam" x-model="jabatan" required 
                                    class="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 outline-none transition bg-gray-50 focus:bg-white focus:border-[#9d1b1b] focus:ring-4 focus:ring-[#9d1b1b]/10 cursor-pointer appearance-none"
                        ...
                        {{-- Unit --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Unit Kerja <span class="text-red-600">*</span></label>
                        ...
                        {{-- Keperluan --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Keperluan <span class="text-red-600">*</span></label>
                ...
                <div class="bg-red-50/50 p-6 rounded-2xl border border-red-100">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        <div>
                            <h3 class="text-lg font-bold text-[#9d1b1b] flex items-center gap-3"><i class="fas fa-box-open"></i> Daftar Arsip</h3>
                            <p class="text-xs text-red-800/70 mt-1">Kelola daftar arsip yang dipinjam.</p>
                        </div>
                        <button type="button" @click="openModal()" 
                            class="w-full md:w-auto px-5 py-2.5 bg-[#9d1b1b] text-white text-sm font-bold rounded-xl hover:bg-[#801010] shadow-md transition flex items-center justify-center gap-2">
                                    ...
                                    <tr class="hover:bg-red-50 transition">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <div class="flex items-center justify-center">
                                                ...
                                                <span x-show="item.media === 'Softfile'" class="text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wide bg-red-50 text-red-800 border border-red-200">Softfile</span>
                                            </div>
                                        </td>
                ...
                {{-- BAGIAN 3: BUKTI --}}
                <div>
                    <h2 class="text-lg font-bold text-[#9d1b1b] border-b border-gray-100 pb-3 mb-6 flex items-center gap-3"><i class="fas fa-paperclip text-[#9d1b1b]"></i> Bukti Peminjaman <span class="text-red-500">*</span></h2>
                    ...
                    @if($editData->bukti_peminjaman)
                        ...
                        <div x-show="!deleteOldFile" class="mb-6 bg-red-50 border border-red-100 rounded-xl p-4 transition-all">
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 gap-2">
                                <div><p class="text-xs font-bold text-red-800 uppercase tracking-wide mb-1">File Tersimpan Saat Ini</p><p class="text-xs text-red-600">File ini akan tetap ada kecuali Anda menghapusnya.</p></div>
                                <button type="button" @click="deleteOldFile = true" class="w-full md:w-auto text-xs font-bold text-red-600 hover:text-red-800 flex items-center justify-center gap-1 bg-white px-3 py-1.5 rounded border border-red-200 hover:bg-red-50 transition shadow-sm"><i class="fas fa-trash-alt"></i> Hapus Semua File Lama</button>
                            </div>
                            <div class="flex flex-wrap gap-2">
                                @foreach($existingFiles as $file)
                                    <a href="{{ asset($file) }}" target="_blank" class="flex items-center gap-2 px-3 py-2 bg-white border border-red-200 rounded-lg text-sm font-medium text-gray-700 hover:text-red-700 hover:border-red-400 transition shadow-sm"><div class="bg-red-50 p-1 rounded text-red-600"><i class="fas fa-file-check"></i></div><span class="truncate max-w-[150px]">{{ basename($file) }}</span></a>
                                @endforeach
                            </div>
                        </div>
                        <div x-show="deleteOldFile" class="mb-6 bg-red-50 border border-red-200 rounded-xl p-4 flex justify-between items-center animate-fade-in-up">
                            <div class="flex items-center gap-3"><div class="bg-red-100 p-2 rounded-full text-red-600"><i class="fas fa-exclamation-triangle"></i></div><div class="text-sm font-bold text-red-800">File lama akan dihapus.</div></div>
                        ...
                        <template x-for="(file, index) in files" :key="file.id">
                            <div class="flex items-center gap-3 animate-fade-in-up">
                                <div class="flex-1 relative group">
                                    <label class="flex items-center justify-between w-full px-4 py-3 bg-white border border-gray-300 rounded-lg cursor-pointer hover:border-red-500 hover:ring-1 hover:ring-red-200 transition shadow-sm group">
                                        <div class="flex items-center gap-4 overflow-hidden"><div class="bg-red-100 text-red-800 w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 border border-red-200"><i class="fas fa-file-alt text-lg"></i></div><div class="flex flex-col overflow-hidden"><span class="text-sm font-bold text-gray-800 truncate" x-text="file.name ? file.name : 'Upload File Tambahan'"></span><span class="text-xs text-gray-500" x-text="file.name ? 'Siap diupload' : 'Klik untuk memilih file'"></span></div></div>
                                ...
                                <button type="button" @click="removeFile(index)" class="w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-lg border border-red-200 text-red-600 bg-white hover:bg-red-100 hover:text-red-900 hover:border-red-400 transition shadow-sm" x-show="files.length > 1 || file.name"><i class="fas fa-trash-alt text-lg"></i></button>
                            </div>
                        </template>
                    </div>
                    <button type="button" @click="addFile()" class="mt-5 w-full py-3 border-2 border-dashed border-red-300 rounded-xl text-red-700 font-bold hover:bg-red-50 hover:border-red-500 transition flex items-center justify-center gap-2 group"><i class="fas fa-plus-circle group-hover:scale-110 transition"></i><span>Tambah File Lain</span></button>
                </div>
            </div>
            ...
            <div class="bg-gray-50 px-8 py-6 border-t border-gray-100 flex justify-end gap-4">
                ...
                <button type="submit" class="px-8 py-3 bg-[#9d1b1b] text-white rounded-xl font-bold shadow-lg hover:bg-[#7a1515] hover:shadow-xl transition transform hover:-translate-y-0.5 flex items-center gap-2"><i class="fas fa-save"></i> Simpan Perubahan</button>
            </div>
        </form>
